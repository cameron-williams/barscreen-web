...
$(document).ready(function () {
  ...
  $("#loop_name")[0].value = loop_name;

  // Blank image after 10s to handle image update.
  setTimeout(function () {
    $("#loop_image")[0].value = null;
  }, 3000);

  $(image_creator);

  // Add existing loop playlist to playlist column.
  $.each(loop_playlist, function (index, value) {
    var playlist_item = '<li><div><img src="' + value.image_url + '"/></div><div><span id="li_id">' + value.id + '</span><h4>' + value.name + '</h4><h5 id="li_type">' + value.type + '</h5></div></li>';
...
$(document).delegate(".playlist_add", 'click', function () {
  ...
  // Set value on form input element.
  loop_data_element.value = JSON.stringify({ "data": loop_data })
  var playlist_item = '<li><div><img src="' + loopItem_img + '"/></div><div><span id="li_id">' + loopItem_id + '</span><h4>' + loopItem_name + '</h4><h5 id="li_type">' + loopItem_type + '</h5></div></li>';
