...
from barscreen.forms.loop import (NewLoopFormDashboard, UpdateDashboardLoopForm)
from barscreen.services.google_clients import GoogleStorage
from barscreen.services.imaging import (b64_image_string_to_file ,screencap_from_video)
...
def edit_loop(loop_id):
    ...
    return render_template("dashboard/edit_loop.html", form=form, loop_playlist=json.dumps(loop_playlist), current_loop=current_loop, current_user=current_user, trends=channel_categories["trends"], entertainments=channel_categories["entertainment"], sports=channel_categories["sports"], news=channel_categories["sports"])
...
def add_loop():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash("Unable to complete request, please try again later.", category="error")
            abort(400)
        ...
        # Handle Loop form post.
            ...
            uploaded_file = b64_image_string_to_file(loop_form.loop_image.data, loop_form.loop_name.data)
            if not uploaded_file:
                flash("Error creating loop, please try again later.", category="error")
                abort(400)
            ...
            try:
                ...
                playlist_data = json.loads(loop_form.loop_data.data).get("data")
                ...
                logging.error("Error uploading loop for user {}: {} {}".format(current_user, type(err), err))
                flash("Error creating loop, please try again later.", category="error")
                abort(400)
